...
\begin{enumerate}
  \item \textbf{Consistency varies widely across models and temperatures.}
    Some models achieve perfect plan consistency ($J = 1.000$) at all
    temperatures, while others drop below $J = 0.5$ at higher settings.
  ...
  \item \textbf{The \gls{eu}-sovereign flagship matches proprietary models
    ...
    and \modelname{Sonnet~4.6}. The gap only appears above the recommended
    operating range.
  ...
  \item \textbf{No single metric captures the full picture.}
    ...
    ($\rho = 0.285$ to $0.560$). BERTScore misses plan-level changes.
...
scores would clarify how much trust to place in automated coherence
assessment.
