...
\begin{table}[h]
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    \emph{self-empowerment} &
      ...
      reinterpreting existing elements, which is \emph{cognitive reframe}.) \\
    \emph{safety} &
...
\begin{description}
  \item[v1 (8 categories):] Included \emph{empowerment} and
    \emph{mastery} as separate categories. These two consumed 87.7\% of
  ...
  \item[v2 (7 categories):] Merged \emph{empowerment} and
    \emph{mastery} into \emph{agency}. The merged category achieved
    100\% dominance.
  \item[v3 (7 categories):] Split \emph{agency} along the mechanism
    dimension into \emph{confrontation} (external action) and
    \emph{self-empowerment} (internal transformation). Sharpened all
    descriptions to focus on mechanism, not outcome.
  \item[v4 (6 categories):] Merged \emph{emotional regulation} into
    \emph{sensory modulation}. In smoke testing (120 trials),
    \emph{emotional regulation} appeared once (0.4\%). In single-turn
...
\section{Coherence Judge Prompt}
...
\begin{figure}[h]
  ...
  \caption{Mean BERTScore F1 by model and temperature, grouped by family.
    ...
    ($\rho = -0.467$), but BERTScore is insensitive to strategy
...
\begin{figure}[h]
  ...
  \caption{Jaccard by model and vignette, one panel per temperature.
    Some vignettes show lower consistency for individual models, though
    the overall vignette effect on Jaccard is not statistically
    significant ($p = 0.094$).}
