...
All results are reported at the second rescripting turn
(\techil{slice\_2}), aggregated across six vignettes. Each data point
represents the mean of twenty independent trials. Supplementary figures
appear in \autoref{app:results}.
...
frequencies differ. \emph{confrontation} and \emph{self-empowerment}
...
strategies will score high on Jaccard regardless of temperature. A model
that draws from a wider repertoire has more room to vary.
...
\begin{figure}[t]
  \centering
  \makebox[\textwidth]{\includegraphics[width=1.05\textwidth]{figures/fig_5_2_jaccard.pdf}}
...
Modal-set agreement (the proportion of trials selecting the exact same
strategy combination) confirms these patterns and is reported in
\autoref{app:results}.

The two proprietary models, \modelname{GPT-5.4} and
\modelname{Sonnet~4.6}, show the most stable temperature profiles. Their
mean Jaccard stays between $0.82$ and $0.92$ across all five temperature
points with no significant trend. Both also show narrower strategy
distributions (\autoref{fig:strategy-dist}), which likely contributes.

Most open-weight models peak at low temperatures.
\modelname{Qwen~3.5~122B} and \modelname{OLMo~3.1~32B} reach $J = 1.0$
at $T = 0.0$ but decline at higher settings. The Mistral family behaves
differently: all three models peak between $T = 0.075$ and $T = 0.15$
rather than at $T = 0.0$. \modelname{Mistral~Small~3.2} reaches
$J = 0.938$ at $T = 0.075$, \modelname{Mistral~Large~3} reaches
$J = 0.848$ at $T = 0.15$, and \modelname{Mistral~Small~4} reaches
$J = 0.881$ at $T = 0.15$.

This means greedy decoding is not always optimal. Several models score
higher at a low non-zero temperature:
\modelname{Mistral~Small~4} ($0.777 \to 0.881$ at $T = 0.15$),
\modelname{Mistral~Large~3} ($0.796 \to 0.848$ at $T = 0.15$), and
\modelname{Llama~3.3~70B} ($0.719 \to 0.775$ at $T = 0.075$). Whether
these differences reflect a genuine effect or sample variation ($n = 6$
vignettes per cell) cannot be determined from these results alone.

Temperature sensitivity also varies across models.
Per-model Spearman correlations between Jaccard and temperature
(\autoref{tab:temp-spearman}) reveal three groups.
\modelname{OLMo~3.1~32B} ($\rho = -0.576$, $p = 0.001$),
\modelname{Qwen~3.5~122B} ($\rho = -0.560$, $p = 0.001$), and
\modelname{Qwen~3.5~27B} ($\rho = -0.455$, $p = 0.012$) show
...
The pooled correlation across all models is $\rho = -0.206$ ($p < 0.001$).
...
than for Jaccard ($\rho = -0.467$, $p < 0.001$): rising temperature
changes response wording more than it changes strategy selection.
However, BERTScore is insensitive to strategy changes: a model that
switches from \emph{confrontation} to \emph{cognitive reframe}
produces surface text similar enough that BERTScore does not detect the
difference. BERTScore therefore serves as a sanity check for response
style rather than a measure of decision stability. Full results appear
in \autoref{app:results}.



% previously: Plan-Response Coherence (Method 3)
\section{Plan-Response Coherence (Method 3)}
\label{sec:res-coherence}
% Mean coherence score by model x temperature
% Per-strategy scoring distributions (absent / partial / implemented)
% [GRAPH] Mean coherence per model x temperature
% [GRAPH] Stacked bar: absent / partial / implemented per strategy category

\autoref{fig:coherence} shows plan-response coherence by temperature and
per model. Coherence measures whether the model's response implements the
strategies it declared in its plan.

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_5_6_coherence.pdf}
  \caption{Plan-response coherence by temperature (left) and overall by
...
The central finding is that coherence is temperature-independent. The
pooled Spearman correlation between coherence and temperature is
$\rho = -0.083$ ($p = 0.154$), not significant. Models do not get worse
...
implement whichever strategies they select equally well (stable coherence).
...
Kruskal-Wallis tests confirm that models differ significantly on coherence
($p < 0.001$). \modelname{Mistral~Small~4} shows the lowest mean
coherence, consistent with its erratic strategy selection at extreme
...
\begin{figure}[t]
  \centering
  \includegraphics[width=1.0\textwidth]{figures/fig_5_5_correlations.pdf}
  \caption{Plan consistency (mean Jaccard) versus response similarity
    (mean BERTScore F1) per model, pooled across temperatures.
...
The moderate Jaccard--BERTScore correlation ($\rho = 0.508$) indicates
...
for one metric to substitute for the other. The weak Jaccard--Coherence
correlation ($\rho = 0.217$) confirms that plan consistency and
plan-response coherence measure different properties.

\autoref{fig:correlations} shows how the models cluster.
...
lower-left: low plan consistency and low response similarity.
...
Kruskal-Wallis tests show a marginally significant vignette effect on
BERTScore ($p = 0.033$) but not on Jaccard ($p = 0.094$). Patient
profile affects response wording more than strategy selection. Full
...
\begin{figure}[t]
  \centering
  \hspace*{-0.3cm}\includegraphics[width=1.02\textwidth]{figures/fig_slice_depth_metrics.pdf}
  \caption{Stability by conversation depth for \modelname{Mistral~Large~3}.
    ...
    slice~3 onward. BERTScore and coherence remain flat across depths.}
...
($J > 0.93$). BERTScore and coherence remain flat across all depths,
