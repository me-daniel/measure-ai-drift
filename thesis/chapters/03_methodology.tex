...
\begin{enumerate}
  ...
  \item The \textbf{router agent} classifies the current \gls{irt} stage
    ...
    recording, rescripting, or rehearsal phase.
...
This three-agent architecture separates the therapist's strategy
decisions from stage classification and patient simulation. The router's
stage classification controls which system prompt the therapist receives,
ensuring that responses are appropriate to the current protocol phase.
...
trace. \textcite{W4K3ARI5} find that adding mistakes to or paraphrasing the
\gls{cot} often does not change the final answer, suggesting that models
do not always rely on their stated reasoning.
...
\techil{empowerment}, \techil{mastery}) produced extreme skew because
...
can cause looping or degraded output for these models
\parencite{W2RFM6E4}. Evaluation
...
(linear-weighted), indicating substantial agreement
\parencite{LVLA9LZU}. Exact agreement is 89.2\%. All 13 disagreements
go in one direction: the human annotator scored higher than the judge,
indicating that the judge is conservative rather than lenient. This
asymmetry is the safe direction for a validation check.
% NOTE: low-scoring trials were also manually inspected during development.
% Zero scores were consistently caused by judge output formatting failures,
% not genuine misalignment. No structured log was kept.
Additionally, the rare trials with zero coherence scores were flagged.
In all observed cases, a zero score indicated a malformed judge output rather than genuine therapeutic misalignment. The
pipeline reruns zero-scored judge calls once before inclusion and reliablely fixes formatting issues.
...
\begin{itemize}
  ...
  \item $T = 0.15$: recommended temperature for Mistral models.
    \modelname{Mistral Small 3.2} sets $T = 0.15$ explicitly in its
    HuggingFace generation configuration. \modelname{Mistral Large 3} and
    \modelname{Mistral Small 4} lack a generation configuration default;
    their model cards recommend temperatures below 0.1 for production use,
    while code examples on the same cards use $T = 0.15$. Because no
    authoritative single default exists for these two models, 0.15 was
    chosen to match the verified \modelname{Small 3.2} value and the
...
summarised with medians and interquartile ranges. BERTScore F1 and coherence
