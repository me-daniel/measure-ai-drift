...
\begin{figure}[t]
  \centering
  \hspace*{0.4cm}\makebox[\textwidth]{\includegraphics[width=1.2\textwidth]{figures/fig_3_1_pipeline.pdf}}
...
\begin{figure}[t]
  \centering
  \makebox[\textwidth]{\includegraphics[width=1.1\textwidth]{figures/fig_3_3_eval_stack.pdf}}
...
\begin{figure}[t]
  \centering
  \makebox[\textwidth]{\includegraphics[width=1.2\textwidth]{figures/fig_3_2_measurement.pdf}}
...
Rare trials with zero coherence scores were flagged separately.
In all observed cases, a zero score indicated a malformed judge output rather than genuine therapeutic misalignment. The
pipeline reruns zero-scored judge calls once before inclusion and reliably fixes formatting issues.
...
\begin{description}
  \item[$T = 0.0$] (greedy decoding) serves as a deterministic upper
    bound on stability. No reproducibility seed is set, as not all
    vendors support one. Even where a seed is available, GPU
    parallelisation and dynamic batching introduce floating-point
    non-determinism that makes output-level reproducibility best effort
    at most \parencite{HITR7PL4}. The twenty-trial repetition design
    captures this variance directly, following \textcite{WA4QD3UE}, who
    use the same 20-trial protocol at $T = 0.0$ to quantify \gls{llm}
    stability.
