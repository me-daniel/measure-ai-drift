...
The full design crosses 11 models $\times$ 6 vignettes $\times$ 5 temperatures $\times$ 20 trials at slice~2, i.e.\ 600 trials per model and 6{,}600 trials in total. Ten models were collected 2026-03-21 to 2026-03-24; Mistral Medium~3.5 on 2026-07-09 (a \texttt{run\_date} column in the released data marks this).
...
Model endpoints are pinned by OpenRouter model IDs; exact IDs, run dates, prompts, and aggregated per-condition results are released with the code. The frozen histories and per-trial raw outputs are retained by the author and available on request; a public release is planned. Two caveats: OpenRouter routes requests to third-party inference providers whose serving stacks (e.g.\ quantisation) may differ between requests and between the March and July collections, and the judge endpoints are versioned previews. Both are discussed in Section~\ref{sec:limitations}.
