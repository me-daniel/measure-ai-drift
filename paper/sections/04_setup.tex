...
Exact model IDs, prompts, and aggregated per-condition results are released with the code at \url{https://github.com/me-daniel/measure-ai-drift}. The main set was collected 2026-03-21 to 2026-03-24, Mistral Medium~3.5 on 2026-07-09, and Command~A on 2026-07-28. A \texttt{run\_date} column in the released data records the collection date of every run, and the frozen histories and per-trial raw outputs are available from the author on request. The original collections used OpenRouter for all models. While not every provider has each model, it is important to pin one model always to the same provider. Since OpenRouter silently drops temperature for GPT-5.1 and later, the data are reruns with the direct OpenAI API, which accepts temperature when reasoning is disabled. The judge endpoints are versioned previews. Section~\ref{sec:limitations} discusses these caveats.
