...
\begin{table}[t]
  ...
  \begin{tabular}{lll}
    ...
    Model & Size & License \\
    \midrule
    Mistral Small 3.2 & 24B dense & Apache 2.0 \\
    Mistral Small 4 & 119B MoE (6.5B act.) & Apache 2.0 \\
    Mistral Medium 3.5 & 128B dense & modified MIT \\
    Mistral Large 3 & 675B MoE (41B act.) & Apache 2.0 \\
    Qwen 3.5 27B & 27B dense & Apache 2.0 \\
    Qwen 3.5 122B & 122B MoE (10B act.) & Apache 2.0 \\
    Qwen 3.5 397B & 397B MoE (17B act.) & Apache 2.0 \\
    OLMo 3.1 32B & 32B dense & Apache 2.0 (full provenance) \\
    Llama 3.3 70B & 70B dense & Llama license \\
    Command A & 111B dense & CC-BY-NC \\
...
Exact model IDs, prompts, and aggregated per-condition results are released with the code at \url{https://github.com/me-daniel/measure-ai-drift}. The main set was collected 2026-03-21 to 2026-03-24, Mistral Medium~3.5 on 2026-07-09, Command~A on 2026-07-28, and the bridge re-collection (Section~\ref{sec:robustness-results}) on 2026-07-30. A \texttt{run\_date} column in the released data records the collection date of every run, and the frozen histories and per-trial raw outputs are available from the author on request. The original collections used OpenRouter with its default provider routing. The later collections pin each model to a single serving provider and reject requests whose parameters cannot be honoured. Because OpenRouter silently drops \texttt{temperature} for GPT-5.1 and later, the released GPT-5.4 data come from a re-collection through the direct OpenAI API, which accepts temperature when reasoning is disabled. The judge endpoints are versioned previews. Section~\ref{sec:limitations} discusses these caveats.
