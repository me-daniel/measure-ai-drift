...
The measurement itself carries residual variance from the serving side. OpenRouter routes requests to third-party providers, so quantisation and batching may differ between requests and between the March and July collections. The judge endpoints are versioned previews whose revision may likewise differ between the two collections, and the human validation ($\kappa=0.777$) covers 60 trials, about 1\% of the total. Judge reliability at scale remains an open question \citep{zheng2023judging}.
