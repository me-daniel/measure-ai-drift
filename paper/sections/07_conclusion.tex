...
\begin{enumerate}
  \item Consistency varies widely across models. Median decision consistency spans 0.69 to 1.00 between models, while the six patient vignettes show no significant effect, an absence of evidence rather than demonstrated invariance.
  ...
  \item Greedy decoding is not reliably optimal. Ten of twelve models reach their best mean consistency at a low non-zero temperature, and the Mistral family peaks at its vendor default, though with six vignettes per condition these optima are suggestive rather than confirmed. Deployments should verify stability at the vendor-recommended setting rather than assume $T{=}0.0$ is safest.
